...
import {
  ...
  CategoryListTopParams,
  CategoryListTopResponse,
  CategoryRetrieveProductsParams,
  CategoryRetrieveProductsResponse,
} from './resources/categories/categories';
import {
  CreateThread,
  RunError,
  ThreadCreateParams,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['SPARK_AI_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['SPARK_AI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SPARK_AI_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Spark AI API.
 */
export class SparkAI {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Spark AI API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['SPARK_AI_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['SPARK_AI_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SPARK_AI_BASE_URL'),
    apiKey = readEnv('SPARK_AI_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? SparkAI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SPARK_AI_LOG'), "process.env['SPARK_AI_LOG']", this) ??
      defaultLogLevel;
  ...
  static SparkAIError = Errors.SparkAIError;
...
SparkAI.Health = Health;
SparkAI.Search = Search;
SparkAI.Chat = Chat;
SparkAI.Evaluations = Evaluations;
SparkAI.Threads = Threads;
SparkAI.Categories = Categories;

export declare namespace SparkAI {
